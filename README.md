...
本项目目前按个人硬件项目公开整理，资料可能还不完备。仓库中的工程文件、源码压缩包、照片和测试说明主要用于记录设计思路和阶段性验证结果，不代表可以直接用于量产、商业设备或安全关键场景。复现前请自行核对原理图、BOM、封装、电源路径、功率器件参数和固件配置；涉及电池、高电压或大电流负载时，请先使用限流电源和低功率负载验证。电子开关不应作为唯一的保险、急停或过流保护手段，欢迎同好结合实际样板和测试数据继续校正。
